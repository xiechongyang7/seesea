...
import com.seesea.seeseacommon.redis.RedisKey;
...
import com.seesea.seeseasms.mapper.SmsChannelPoMapper;
import com.seesea.seeseasms.mapper.SmsSendAndCheckPoMapper;
...
public class SmsServiceImpl extends BaseService implements ISmsService {
    ...
    @Autowired
    private SmsChannelPoMapper smsChannelPoMapper;
    @Autowired
    private SmsSendAndCheckPoMapper smsSendAndCheckPoMapper;
    ...
    public Map<String, Object> send(SendSmsVo sendSmsVo) throws BizException {
        ...
        SmsSendAndCheckPo smsSendAndCheckPo = new SmsSendAndCheckPo();
        //设置短信id
        sendSmsVo.setSmsId(IdGenerate.getSmsId());
        SmsChannelPo smsChannelPo = null;
        String num = null;
        try {
            //1 判断是否发送
            ...
            if(!StringUtil.isNull(num)){
                if(Integer.valueOf(num) == limit_send_num){
                    throw new BizException(ResultCode.ER_1206, String.format(ResultCode.ER_1206_MSG,limit_send_num)+"用默认的吧1234");
                }
            }
            ...
            if (RedisUtils.exists(String.format(RedisKey.PHONE_SEND_SMS,phone))) {
                ...
                throw new BizException(ResultCode.ER_1201, ResultCode.ER_1201_MSG);
            }


            //2 找个能发的
            List<SmsChannelPo> smsChannelPos = smsChannelPoMapper.selectACanUseChannel();
            if (smsChannelPos.size() == 0) {
                logInfo(sendSmsVo.getReqId(), phone, "没有能发的通道了");
                throw new BizException(ResultCode.ER_1202, ResultCode.ER_1202_MSG);
            }
            /**
             * 找个有免费短信的
             */
...
//            }
            ...
            if(fieldMap.isEmpty()){
                ...
                if(isSendTaday){
                    throw new BizException(ResultCode.ER_1209,ResultCode.ER_1209_MSG);
                }
                throw new BizException(ResultCode.ER_1207,ResultCode.ER_1207_MSG);
            }
...
//                检查校验次数 防止暴力破解
                if(Integer.valueOf(checkNum)>=limit_check_num){
                    ...
                    throw new BizException(ResultCode.ER_1208,String.format(ResultCode.ER_1208_MSG,3));
                }else {
                    RedisUtils.hincrby(String.format(RedisKey.PHONE_SEND_SMS,phone),RedisKey.VCODE_CHECK_FILED,1);
                }
                throw new BizException(ResultCode.ER_1210,ResultCode.ER_1210_MSG);
            }
            //3 成功就即时删除redis的信息
            RedisUtils.del(String.format(RedisKey.PHONE_SEND_SMS,phone));
            ...
            result.put("msg", ResultCode.SUCCESS_MSG);
